...
Well — actually we still have a problem with the current setup. Nothing is stopping anyone to use the `highlight` color (meant for background surfaces) on a text element, for example.

```html
<p className="text-highlight ...">Special Deal</p>
```
